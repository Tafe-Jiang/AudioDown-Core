...
| 阶段 4：凭据金库 | 进行中 | Task 1-12 已完成并通过复审（12/20）：凭据领域与协议、作用域校验、Node SDK、AES-256-GCM 加密、本机主密钥、加密凭据持久化、凭据金库服务、受限 HTTP 传输、临时 Cookie Jar，以及受认证、严格限界和脱敏的 Core Unix Socket 代理后端；下一项为 Task 13 固定 Gateway 隔离 |
...
- 不自动检查或安装插件更新。
- 不包含任何真实平台插件或真实下载能力。
- 阶段 4 当前已验收 Task 1-12；受认证代理后端只提供 Core 内部 Unix Socket 边界，不包含真实 Cookie 获取或真实平台登录。
