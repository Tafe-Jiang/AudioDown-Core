...
这里的安全边界是：Core 不提供任何凭据明文导出 API，插件 RPC、HTTP
响应、日志和持久化元数据都不能直接包含 Core 保存的 Cookie 或 Token。
用户显式授权的内容插件只获得限定作用域和目标 Origin 的账号操作能力，
不获得明文导出能力。Core 必须阻断直接秘密回显并对响应做固定边界过滤，
但无法在通用 HTTP 协议上证明一个与插件串通的远端服务没有对秘密做任意
编码或语义转换；这种远端串通不属于 1.0 的威胁模型，用户授权与撤销是
对应的信任边界。

凭据记录必须保存其规范化目标 Origin。内容插件授权的 Origin 必须同时
属于内容插件声明的目标、凭据记录的目标和用户明确选择的交集。手工
Cookie 导入必须选择确切 Origin，由 Core 生成 host-only、Secure、Path=/
的 Cookie 绑定，不能只依赖作用域或插件当前 manifest 推断目标。

1.0 的金库封装和代理注入模型支持 Cookie 与 Token 两类密文。第一阶段
公开导入流程只接受 Cookie；Token 只能由可信 Core 服务写入，不能由插件
提交明文或通过公开接口导入。虚拟测试可以通过内部测试适配器验证 Token
加密、注入和防泄漏，真实 Token 获取流程不在 1.0 第一阶段范围内。
